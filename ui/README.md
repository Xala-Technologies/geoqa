...
The view model comes from `src/report/view.ts`, which is covered at 100% like the rest of the
...
`pnpm ui:build` emits static files into `ui/dist`. Copy them beside `dashboard.json` and the
