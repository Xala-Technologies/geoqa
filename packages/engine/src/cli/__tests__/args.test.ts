...
describe("USAGE", () => {
  ...
  it("says assist explain is after judgement and bills Max, not the API", () => {
    expect(USAGE).toContain("assist explain");
    expect(USAGE).toContain("claude -p");
    expect(USAGE).toContain("Max login");
    expect(USAGE).toContain("ANTHROPIC_API_KEY");
    expect(USAGE).toContain("measurement path");
  });
  ...
  it("says the server is the operator console and that Live is not remote control", () => {
    ...
    expect(USAGE).toContain("Watch writes inputs/tenants/");
