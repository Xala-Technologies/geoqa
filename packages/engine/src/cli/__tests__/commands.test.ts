...
import { defaultEvidenceRoot, experimentsRoot, findRepoRoot, journeysRoot, profilesRoot, tenantsRoot } from "../../repo.js";
...
describe("defaultDeps", () => {
  it("builds real defaults when nothing is overridden", () => {
    ...
    expect(d.evidenceRoot).toBe(defaultEvidenceRoot(repoRoot));
...
describe("list commands", () => {
  ...
  it("marks an invalid file INVALID rather than throwing the whole listing away", () => {
    ...
    mkdirSync(profilesRoot(brokenRoot), { recursive: true });
    mkdirSync(journeysRoot(brokenRoot), { recursive: true });
    writeFileSync(path.join(profilesRoot(brokenRoot), "bad.yaml"), "id: x\n");
    writeFileSync(path.join(journeysRoot(brokenRoot), "bad.yaml"), "id: x\n");
...
describe("assistExplain", () => {
  const writeRun = (runId: string, verdict = "FAIL"): void => {
    mkdirSync(path.join(evidenceRoot, runId), { recursive: true });
    writeFileSync(
      path.join(evidenceRoot, runId, "run.json"),
      JSON.stringify({
        runId,
          ...
          steps: [
            {
              index: 0,
              action: "assert",
              label: "search did not 500",
              outcome: "failed",
              severity: "critical",
              detail: "no-http-5xx: expected 0, observed 1",
              expected: "0",
              observed: "1",
              durationMs: 40,
            },
  ...
  it("does not call Claude when the package is missing", async () => {
    const assistComplete = vi.fn();
    const result = await assistExplain(deps({ assistComplete }), "run_missing");
    expect(result.ok).toBe(false);
    expect(assistComplete).not.toHaveBeenCalled();
  });

  it("sends the brief and returns the draft, billed as Max", async () => {
    writeRun("run_1");
    const result = await assistExplain(
      deps({
        assistComplete: async (prompt) => {
          expect(prompt).toContain("search did not 500");
          expect(prompt).toContain("Do not invent");
          return { ok: true, text: "Title: search 500s on explore" };
        },
      }),
      "run_1",
    );
    expect(result).toMatchObject({
      ...
      draft: "Title: search 500s on explore",
    });
    if (result.ok) expect(result.brief).toContain("FAIL: explore");
  });

  it("surfaces a Claude failure without inventing a draft", async () => {
    writeRun("run_1");
    const result = await assistExplain(
      deps({
        assistComplete: async () => ({ ok: false, failure: { kind: "spawn", detail: "claude CLI not found on PATH" } }),
      }),
      "run_1",
    );
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.error).toContain("spawn");
  });
});
    ...
    await experimentRun(
      ...
      async (deps2, o, i) => {
        if (i === 1) {
          ...
          paths = readFileSync(path.join(experimentsRoot(evidenceRoot), "EXP-001-geo-ip", "results.jsonl"), "utf8");
        }
        return { i };
      },
      summariser,
...
describe("tenant-scoped profiles and journeys", () => {
  ...
  it("prefers the TENANT's file over the shared one, and falls back for everything else", () => {
    ...
    expect(shared.value).toContain(journeysRoot(repoRoot));
  });
  ...
  it("treats a tenant with no data directory as using the shared set", () => {
    ...
    expect(result.value).toContain(profilesRoot(repoRoot));
...
describe("what the CLI says when a file on disk does not load", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(profilesRoot(root), { recursive: true });
    mkdirSync(journeysRoot(root), { recursive: true });
    mkdirSync(tenantsRoot(root), { recursive: true });
  });
  ...
  it("LISTS a malformed tenant as INVALID rather than dropping it from the list", () => {
    ...
    writeFileSync(path.join(tenantsRoot(root), "acme.yaml"), "id: acme\nname: [not a string\n");
  ...
  it("names the profile and the reason, not just \"invalid\"", () => {
    writeFileSync(path.join(profilesRoot(root), "oslo-broken.yaml"), "id: oslo-broken\nmarket: 12\n");
  ...
  it("refuses a keyword seed file that does not parse, rather than researching nothing", async () => {
    ...
    mkdirSync(path.join(tenantsRoot(root), "acme"), { recursive: true });
    writeFileSync(path.join(tenantsRoot(root), "acme", "keywords.yaml"), "seeds: [\n");
    const tenant: Tenant = {
      id: "acme",
      name: "Acme",
      markets: ["oslo"],
...
describe("refusing before spending anything", () => {
  it("names EVERY broken file in a matrix, not the first one", () => {
    ...
    try {
      mkdirSync(profilesRoot(root), { recursive: true });
      mkdirSync(journeysRoot(root), { recursive: true });
      writeFileSync(path.join(profilesRoot(root), "oslo-desktop.yaml"), "id: oslo-desktop\nmarket: 7\n");
      writeFileSync(path.join(journeysRoot(root), "landing-page.yaml"), "id: landing-page\nsteps: 4\n");
