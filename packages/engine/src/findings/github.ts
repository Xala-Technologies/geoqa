...
export function loadFiledIssues(evidenceRoot: string, store: FiledStore): { ok: true; issues: FiledIssue[] } | { ok: false } {
  ...
  try {
    ...
    return parsed.success ? { ok: true, issues: parsed.data.issues } : { ok: false };
  } catch {
    return { ok: false };
  }
}
...
export async function fileTickets(input: FileTicketsInput): Promise<FileTicketsResult> {
  ...
  const known = new Set(loaded.issues.map((issue) => issue.key));

  if (input.dryRun === true) {
    return { ...empty, skipped: "dry-run", wouldFile: input.drafts.filter((d) => !known.has(d.key)), already: input.drafts.filter((d) => known.has(d.key)).map((d) => d.key) };
  }
